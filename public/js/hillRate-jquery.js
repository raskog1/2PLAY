...
$(document).ready(function() {
  (function($) {
    var defaults = {
      stars: 3,
      imageStar: {
        default: "img/star-empty.png",
        full: "img/star-full.png",
        half: "img/star-half.png",
      },
      valuesStar: [0, 1, 2],
      nameInput: "rating",
      responsive: false,
    ...
    var methods = {
      init: function(options) {
        ...
        ratings.each(function(index) {
          ...
          var state_unselected = settings.imageStar.state_unselected
            ? settings.imageStar.state_unselected
            : "full";

          var imageStarOnIndex = settings.imageStarOnIndex;
          var valuesStar = settings.valuesStar
            ? settings.valuesStar
            : methods.initialValues(numStar);
            ...
            if (imageStarOnIndex) {
              for (var j = 0; j < imageStarOnIndex.length; j++) {
                ...
                if (thisImageStarOnIndex.index == i) {
                  ...
                  stateUnselected = thisImageStarOnIndex.state_unselected
                    ? thisImageStarOnIndex.state_unselected
                    : "full";
                }
              }
            }
            var percentual = 100 / (numStar + (showSelectedValue ? 1 : 0));
            var styleItem = responsive
              ? 'style="width:' + percentual + '%"'
              : "";
            rating.append(
              '<img data-id="' +
                i +
                '" class="item-rate" data-title="' +
                titleStar +
                '" data-value="[' +
                valStar +
                ']" data-half="' +
          ...
          var styleItem = responsive
            ? 'style="width:' +
              percentual +
              '%;float: right;text-align:center;margin-top:10px;font-size:20px;"'
            : "";
          rating.append(
            ' <div class="selected_value" ' + styleItem + "></div>"
          );
          ...
          if (initialValue >= 0) {
            if (
              methods.existInitValueOfStar(initialValue, settings.valuesStar)
            ) {
    ...
    $.fn.hillRate = function(methodOrOptions) {
      if (methods[methodOrOptions]) {
        ...
        $.error(
          "Method " + methodOrOptions + " does not exist on jQuery.hillRate"
        );
      }
    };
  })(jQuery);
